...
module Metasploit3
  ...
  def initialize(info = {})
    super(merge_info(info,
      'Name'          => 'Windows MessageBox (EBNIDS-evasion PoC version)',
      'Description'   => 'Spawns a dialog via MessageBox using a customizable title, text & icon. Integrates EBNIDS "kernel32 resolution heuristic" evasion',
      'Author'        =>
        [
...
EOS
  ...
  fstenv [esp-0xc]
  xor edx,edx
  mov dl,0x5e	;offset to start_main
...
start_main:
  ...
  ; MOD: Anti-heuristics: include base address resolution code, we can rely on already included find_function
  #{resolutionCode}
