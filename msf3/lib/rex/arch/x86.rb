...
#
#
# Slightly modified x86 generator with additional functions
#================================
...
module X86
  ...
  #
  # Generate a JNZ instruction
  #
  def self.jnz(offset)
    "\x75" + pack_lsb(rel_number(offset, -2))
  end
  ...
  #
  # This method returns the opcodes that compose a je instruction to
  # the supplied relative offset.
  #
  def self.je(addr)
    "\x74" + pack_lsb(rel_number(addr))
  end

  #
  # This method returns the opcodes that compose a jne instruction to
  # the supplied relative offset.
  #
  def self.jne(addr)
    "\x75" + pack_lsb(rel_number(addr))
  end
  ...
  #
  # MOD: This method returns the opcodes that compose a call instruction to the register specified.
  #
  def self.call_reg(reg)
    _check_reg(reg)
    "\xFF" + (0xD0 | reg).chr
  end
  ...
  #
  # MOD: This method generates a pop dword instruction into a register.
  #
  def self.push_reg(dst)
    _check_reg(dst)
    return (0x50 | dst).chr
  end
  ...
  #
  # This method generates the opcodes that set the dst register to the src register.
  #
  def self.mov_reg(dst, src)
    _check_reg(dst)
    _check_reg(src)

    return "\x89" + (0xc0 | (dst + (8 * src))).chr
  end

  #
  # This method generates the opcodes that xor the dst register with the src register.
  #
  def self.xor_reg(dst, src)
    _check_reg(dst)
    _check_reg(src)

    return "\x31" + (0xc0 | (dst + (8 * src))).chr
  end

  #
  # This method generates the opcodes that cmp the dst register with the src register.
  #
  def self.cmp_reg(dst, src)
    _check_reg(dst)
    _check_reg(src)

    return "\x39" + (0xc0 | (dst + (8 * src))).chr
  end
...
# TODO: AND
  ...
  #
  # MOD: Builds an addition instruction using the supplied registers.
  # Improvements:
  #            - Support additional ways of register addition (and support badchars)
  #
  def self.add_reg(dstReg, srcReg)
    _check_reg(dstReg, srcReg)
    return "\x03" + (0xC0 + ((dstReg * 8) + srcReg)).chr
  end

  #
  # MOD: Builds a subtraction instruction using the supplied registers.
  # Improvements:
  #            - Support additional ways of register subtraction (and support badchars)
  #
  def self.sub_reg(dstReg, srcReg)
    _check_reg(dstReg, srcReg)
    return "\x2B" + (0xC0 + ((dstReg * 8) + srcReg)).chr
  end
  ...
  #
  # MOD: This method generates a test register, register instruction.
  #
  def self.test_reg(dstReg, srcReg)
    _check_reg(dstReg, srcReg)
    return "\x85" + (0xC0 + ((srcReg * 8) + dstReg)).chr
  end
